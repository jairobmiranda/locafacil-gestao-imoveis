...
export const CLAUSULAS_VISTORIA: Clausula[] = [
  ...
  },
  {
    ...
    paragrafos: (ctx) => [
      `Apurados danos, ${oLocatario(ctx)} ${verbo(ctx.locatarios, 'deverá', 'deverão')} promover os reparos no prazo de 15 (quinze) dias ou, a critério ${doLocador(ctx)}, ressarcir o custo correspondente mediante orçamento, autorizada a compensação com a garantia prestada.`,
      `Enquanto não concluídos os reparos ou efetuado o ressarcimento, ${verbo(ctx.locatarios, 'permanece', 'permanecem')} ${oLocatario(ctx)} ${verbo(ctx.locatarios, 'responsável', 'responsáveis')} pelo pagamento do aluguel e dos encargos, a título de indisponibilidade do IMÓVEL.`,
    ],
  },
];
...
export const CLAUSULAS_USO: Clausula[] = [
  {
    ...
    paragrafos: (ctx) => [
      ...
      `A infração a esta cláusula constitui falta grave e autoriza a rescisão imediata com despejo, além da multa prevista na cláusula de rescisão.`,
    ],
  },
  {
    id: 'VEDACAO_OBRAS',
  ...
  },
  {
    ...
    caput: (ctx) =>
      `É vedada a permanência de animais no IMÓVEL sem prévia autorização escrita ${doLocador(ctx)}, ressalvados os casos de cão-guia e demais hipóteses de proteção legal obrigatória.`,
  },
  {
    id: 'OCUPANTES',
    versao: 1,
...
export const CLAUSULAS_RESCISAO: Clausula[] = [
  {
    ...
    grupo: 'RESCISAO',
    nivelProtecao: 3,
    baseLegal: 'Lei 8.245/91, arts. 4º e 6º',
    ...
    paragrafos: (ctx) => [
      ...
      `A multa é igualmente devida quando a rescisão decorrer de infração contratual ${doLocatario(ctx)}, sem prejuízo da apuração de perdas e danos e da ação de despejo.`,
      `A denúncia da locação prorrogada por prazo indeterminado, com aviso prévio de 30 (trinta) dias, não enseja multa. A ausência do aviso obriga ao pagamento de importância equivalente a um aluguel e encargos.`,
    ],
  },
  {
    ...
    obrigatoria: true,
    nivelProtecao: 3,
    baseLegal: 'Lei 8.245/91, arts. 9º e 62',
    caput: (ctx) =>
      `Constitui infração contratual grave, autorizando a rescisão de pleno direito e a propositura da competente ação de despejo, o descumprimento de qualquer obrigação prevista neste contrato, em especial a falta de pagamento do aluguel e dos encargos, a alteração da destinação, a sublocação não autorizada, a realização de obras não autorizadas e a perda ou insuficiência da garantia.`,
    paragrafos: (ctx) => [
      `A parte que der causa à rescisão responderá pela multa compensatória prevista neste contrato, sem prejuízo da indenização por perdas e danos efetivamente comprovados.`,
      `${oLocador(ctx)} ${verbo(ctx.locadores, 'poderá', 'poderão')} considerar antecipadamente vencidas todas as obrigações pendentes, cobrando-as de uma só vez.`,
    ],
  },
  {
    id: 'TITULO_EXECUTIVO',
    ...
    grupo: 'RESCISAO',
    nivelProtecao: 3,
    baseLegal: 'CPC, art. 784, III e VIII',
    condicao: (ctx) => ctx.respostas.duasTestemunhas,
    caput: () =>
      `Este contrato, assinado pelas partes e por duas testemunhas, constitui título executivo extrajudicial, nos termos do artigo 784, incisos III e VIII, do Código de Processo Civil, autorizando a execução direta dos valores nele previstos, incluídos aluguéis, encargos, multas e acessórios.`,
  },
];
...
export const CLAUSULAS_PREFERENCIA: Clausula[] = [
  {
    ...
    grupo: 'PREFERENCIA',
    nivelProtecao: 1,
    baseLegal: 'Lei 8.245/91, arts. 27 a 34',
    ...
    paragrafos: (ctx) => [
      ...
      `Alienado o IMÓVEL durante a locação, o adquirente poderá denunciar o contrato no prazo de 90 (noventa) dias contados do registro da venda, salvo se a locação for por prazo determinado, contiver cláusula de vigência em caso de alienação e estiver averbada na matrícula, na forma do artigo 8º da Lei 8.245/91.`,
    ],
  },
];
...
export const CLAUSULAS_GERAIS: Clausula[] = [
  ...
  },
  {
    ...
    caput: (ctx) =>
      `As partes elegem o correio eletrônico e as mensagens instantâneas informados neste instrumento como meios válidos e eficazes de comunicação, notificação e interpelação, reputando-se recebidas as mensagens enviadas aos endereços aqui indicados.`,
    paragrafos: (ctx) => [
      ...
      `Os endereços indicados no preâmbulo servem também para o recebimento de notificações judiciais e extrajudiciais.`,
    ],
  },
  {
    id: 'LGPD',
...
export const CLAUSULAS_FORO: Clausula[] = [
  {
    ...
    obrigatoria: true,
    caput: (ctx) =>
      `As partes elegem o foro da comarca de ${ctx.foro} para dirimir as questões oriundas deste contrato, com renúncia a qualquer outro, por mais privilegiado que seja.`,
  },
];
