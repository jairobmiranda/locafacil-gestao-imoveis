...
import { atualizarContrato, criarContrato, type EstadoFormulario } from './acoes';
...
type ContratoParaEdicao = {
  ...
  chavePix: { id: string } | null;
  diasAvisoEncerramento: number;
  diasAntecedenciaGeracao: number;
  gerarCobrancas: boolean;
  observacoes: string | null;
  emailsCopia: string | null;
  itens: { descricao: string; valor: number; categoria: { id: string } }[];
};

type ItemFormulario = { chave: number; descricao?: string; valor?: number; categoriaId?: string };
...
function Botao({ editando }: { editando: boolean }) {
  ...
  return (
    <button type="submit" className="botao botao-primario" disabled={pending}>
      {pending ? 'Salvando...' : editando ? 'Salvar alterações' : 'Criar contrato'}
    </button>
  );
}
...
export function FormularioContrato({
  ...
  const [estado, acao] = useActionState<EstadoFormulario, FormData>(
    contrato ? atualizarContrato : criarContrato,
    {},
  );
  ...
  return (
    <form action={acao} className="cartao formulario">
        ...
        <div className="grade">
          <label className={erros.dataInicio ? 'campo com-erro' : 'campo'}>
            ...
            <input
              name="dataInicio"
              type="date"
              defaultValue={contrato?.dataInicio.slice(0, 10)}
              required
            />
          </label>
          ...
          <label className={erros.dataFim ? 'campo com-erro' : 'campo'}>
            ...
            <input
              name="dataFim"
              type="date"
              defaultValue={contrato?.dataFim.slice(0, 10)}
              required
            />
          ...
          <label className={erros.valorAluguel ? 'campo com-erro' : 'campo'}>
            Valor do aluguel
            <EntradaValor name="valorAluguel" valor={contrato?.valorAluguel} required />
            {erros.valorAluguel ? (
      ...
      <fieldset>
        ...
        {itens.map((item) => (
          <div className="grade linha-item" key={item.chave}>
            <label className="campo">
              Descrição
              <input name="itemDescricao" maxLength={150} defaultValue={item.descricao} />
            </label>

            <label className="campo">
              Categoria
              <select name="itemCategoria" defaultValue={item.categoriaId ?? ''}>
                <option value="" disabled>
                  Selecione
                </option>
            ...
            <label className="campo">
              Valor
              <EntradaValor name="itemValor" valor={item.valor} />
            </label>
            ...
            <button
              ...
              onClick={() =>
                setItens((atuais) => atuais.filter((atual) => atual.chave !== item.chave))
              }
            >
              Remover
            </button>
        ...
        <button
          type="button"
          className="botao"
          onClick={() => setItens((atuais) => [...atuais, { chave: Date.now() }])}
        >
          Adicionar encargo
        </button>
      </fieldset>
      ...
      <fieldset>
        ...
        <div className="grade">
          <label className="campo">
            Multa por atraso (%)
            <EntradaValor name="percentualMulta" valor={contrato?.percentualMulta ?? 2} />
          </label>

          <label className="campo">
            Juros ao dia (%)
            <input
              name="percentualJurosDia"
              type="number"
              step="0.0001"
              defaultValue={contrato?.percentualJurosDia ?? 0.033}
            />
          </label>
          ...
          <label className="campo">
            ...
            <EntradaValor
              ...
              valor={contrato?.descontoPontualidade ?? 0}
            />
          </label>

          <label className="campo">
            Índice de reajuste
            <select name="indiceReajuste" defaultValue={contrato?.indiceReajuste ?? 'IGPM'}>
          ...
          <label className="campo">
            ...
            <input
              name="intervaloReajusteMeses"
              type="number"
              min={1}
              defaultValue={contrato?.intervaloReajusteMeses ?? 12}
            />
          </label>
        </div>
      </fieldset>
      ...
      <fieldset>
        ...
        <div className="grade">
          <label className="campo">
            Tipo de garantia
            <select name="tipoGarantia" defaultValue={contrato?.tipoGarantia ?? 'NENHUMA'}>
          ...
          <label className="campo">
            Valor da garantia
            <EntradaValor name="valorGarantia" valor={contrato?.valorGarantia} />
          </label>

          <label className="campo">
            Chave Pix
            <select name="chavePixId" defaultValue={contrato?.chavePix?.id ?? ''}>
          ...
          <label className="campo">
            ...
            <input
              name="diasAvisoEncerramento"
              type="number"
              min={0}
              defaultValue={contrato?.diasAvisoEncerramento ?? 90}
            />
          </label>
          ...
          <label className="campo">
            ...
            <input
              name="diasAntecedenciaGeracao"
              type="number"
              min={0}
              defaultValue={contrato?.diasAntecedenciaGeracao ?? 10}
            />
          </label>
        </div>
        ...
        <label className="campo-inline">
          <input
            type="checkbox"
            name="gerarCobrancas"
            defaultChecked={contrato?.gerarCobrancas ?? true}
          />
          <span>
      ...
      <label className={erros.emailsCopia ? 'campo com-erro' : 'campo'}>
        ...
        <input
          name="emailsCopia"
          placeholder="conjuge@exemplo.com; contador@exemplo.com"
          defaultValue={contrato?.emailsCopia ?? ''}
        />
        <small className="texto-suave">Separe por ponto e vírgula</small>
        {erros.emailsCopia ? <span className="mensagem-campo">{erros.emailsCopia}</span> : null}
      </label>

      <label className="campo">
        Observações
        <textarea name="observacoes" rows={2} defaultValue={contrato?.observacoes ?? ''} />
      </label>

      {estado.erro ? <p className="alerta-erro">{estado.erro}</p> : null}

      <div className="acoes-formulario">
        <Botao editando={Boolean(contrato)} />
      </div>
    </form>
  );
}
